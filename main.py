...
# 静态文件：前端 + 图片（Railway 等用 DATA_DIR 时图片在 /data/images，需单独挂载）
STATIC_ROOT = Path(__file__).resolve().parent / "static"
STATIC_ROOT.mkdir(exist_ok=True)
(config.IMAGES_DIR).mkdir(parents=True, exist_ok=True)
# 若图片目录在 STATIC_ROOT 外（如持久化卷），先挂载 /static/images 以便优先命中
try:
    images_resolved = config.IMAGES_DIR.resolve()
    static_resolved = STATIC_ROOT.resolve()
    if not str(images_resolved).startswith(str(static_resolved)):
        app.mount("/static/images", StaticFiles(directory=str(config.IMAGES_DIR)), name="static_images")
except Exception:
    pass
